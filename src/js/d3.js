(function() {
    ...
    D3.tooltip = Backbone.View.extend({
      initialize: function(){
        ...
        this.offset = this.tipper.offset();
        this.offset.top -= this.options.height;
        this.tipper
          .on("mouseover",this.render)
      ...
      ,remove : function(){
        this.tipper.off("mouseover").off("mouseout");
      }
    });
    ...
    D3.bar = D3.extend_base(function(selection,index){
      ...
      */
      ...
      if (!this.is_mini){
        make_legend(sel,this.width);
      } else {
        yAxis.tickValues(y.domain());
        var height = legend.length * 20; 
